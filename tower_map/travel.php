...
$psel = mysql_query ("SELECT * FROM players WHERE location = '$character->location' and user != '$character->user' and room = '300' and lpv > ".(time()-60)." order by level DESC ");
...
function draw_square($link = false, $picture, $id='', $ch_img='images/clear.gif', $locname='', $mobs= array('text'=>''), $markers= array(), $dir='') {
	...
	if ($link) {
		$s = '<td align=center valign=middle style="background:url('.$picture.')"><form action="travel.php" method="post"><input type="hidden" name="destination" value="'.$id.'">'.$image.'</form></td>';
	} else {
...
function getmobs ($loc) {
	...
	$res = mysql_query ("SELECT * FROM players WHERE location = $loc and lpv > ".(time()-60)." and room = '300' order by level DESC ");
...
if ($Attack) {
	...
	if (empty($login)) $msg = "Укажите логин!";
		...
		elseif ($chl['room']!=300) $msg="Для нападния Вам необходимо находится в одной комнате!";
		...
		elseif (((time()-$chl['lpv'])<$bht) && ($chl['battle'] == $chl['last_battle'] || !$chl['battle']) && $chl['rank']==60) $msg="Бот <u>".$chl['user']."</u> еще не восстановил свой уровень жизни!";
...
//echo"
//<center><input type=button class=input value='Нападение' style='WIDTH: 120px' onclick=\"javascript:ShowForm('Нападение','travel.php?Attack=$now','','','1','attack','1','0');\"></center>
//";
//}
echo"
<center><input type=button class=input value='Нападение' style='WIDTH: 120px' onclick=\"javascript:ShowForm('Нападение','travel.php?Attack=$now','','','1','attack','1','0');\">
<input type=button class=input value='Обновить' style='WIDTH: 120px' onclick='window.location.href=\"travel.php?tmp=\"+Math.random();\"\"'></center>
";
