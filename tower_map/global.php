...
function candoquest($char, $quest)
{
	...
	$cando=1;
	$sql="SELECT * FROM players WHERE id=$char LIMIT 1;";
	$res=mysql_query($sql);
...
function who_is_online($location = '')
{
	...
	$result = mysql_query("SELECT * FROM players WHERE $loc user != 'phaos_npc'  ORDER by name ASC");
	...
	if (mysql_num_rows($result) != 0)
	{
		...
		{
			$html .=  '<font color="#009900">|</font><a href="player_info.php?player_name='. $row['user'] . '" target="_blank">' . $row['name'] .  '</a>';
		}
	}
	else
	{
...
function who_is_offline($location = '')
{
	...
	if (mysql_num_rows($result) != 0)
	{
		while ($row = mysql_fetch_assoc($result))
		{
			$html =  '<font color="#009900">|</font><a href="player_info.php?player_name='. $row['user'] . '" target="_blank" style="color=#FFFFFF">' . $row['name'] .  '</a>';
		}
	}
	return $html . '<font color="#009900">|</font>';
}
...
function jsChangeLocation($link){
	if(@DEBUG){
		?>
<a href="<?=htmlSpecialChars($link)?>" style="background: red"><?=htmlSpecialChars($link)?></a>
		<?php
	}else{
		?>
...
function htmlSelect($name,$options,$selected,$vk=0,$ok=1,$size="",$null="",$defaulttext="------"){
	?>
<select name="<?=$name?>" <?=$size?"size=\"$size\"":""?>
...
function nextLocationIdFromRange($location_type,$file=__FILE__,$line=__LINE__){
	...
	$query= "select max(id) from tower_locations where id>=$min and id<$max";
	...
	if( !$id ){
		...
		//        $query= "insert into tower_locations         (id, name, image_path, special, buildings, pass, explore)        values         ($min, 'BEGIN location range($location_type:$min-$max)','images/special.gif',1,0,1,0)          ";
...
function actionButton($label,$action,$fields,$target='content',$method='POST'){
	?>
<form action="<?=$action?>" method="<?=$method?>" target="<?=$target?>"><?php
