...
if( @$_SESSION['charfrom'] != "arena" ) {
	...
	//CAVEAT: users have limited length, so don't use too long names for npc character types
			...
			if($attackingcharacter->hit_points<=0){
				...
				if($damage_multiplier>0) {
					...
					if($character->hit_points <= 0) {
						...
						$req=mysql_query("UPDATE players SET gold = ".$character->gold.", xp = ".$character->xp." WHERE id = '".$character->id."'");
						if (!$req) { showError(__FILE__,__LINE__,__FUNCTION__); exit;}

						$req=mysql_query("UPDATE players SET gold = ".$attackingcharacter->gold.", xp = ".$attackingcharacter->xp." WHERE id = '".$attackingcharacter->id."'");
						if (!$req) { showError(__FILE__,__LINE__,__FUNCTION__); exit;}
						...
						if (! mysql_query("UPDATE players SET location = 4072 WHERE id = '$PHP_PHAOS_CHARID'") ) {  showError(__FILE__,__LINE__,__FUNCTION__); die; }
...
<table width=400 border=1 cellspacing=0 cellpadding=5 align=center>
		...
		<table width="100%" border=0 cellspacing=0 cellpadding=3 align=left>
			<tr>
				<td align=center><input type='button'
					onClick='self.location="combat.php?comb_act=both_attack"'
					value='<? echo $lang_comb["_attt"]; ?>'></td>
				<td align=center><input type='button'
					onClick='self.location="combat.php?comb_act=flee"'
					value='<? echo $lang_comb["_flee"]; ?>'></td>
			</tr>
		</table>
		</td>
	</tr>
	...
	// $result=mysql_query("SELECT id,item_id FROM phaos_char_inventory WHERE user = '".$character->user."' AND type='spell_items'");
	$result=mysql_query("SELECT id,item_id,count(item_id) FROM phaos_char_inventory WHERE user = '".$character->user."' AND type='spell_items' group by item_id ");
	...
	<td>
   	<table width='100%' border=0 cellspacing=0 cellpadding=3 align=left><tr>";
	$result=mysql_query("SELECT id,item_id,count(item_id) FROM phaos_char_inventory WHERE user = '".$character->user."' AND type='potion' group by item_id ");
	if( mysql_num_rows($result) ){
		while(list($id,$item_id,$count) = mysql_fetch_array($result)){
	...
	<tr>
		<td>
		<table width="100%" weight="100%" border=0 cellspacing=0 cellpadding=0
			align=center>
			<tr>
			<?
			if(@$combat_continue){
	...
	<tr>
		<td align=center><input type='button'
			onClick='self.location="combat.php?comb_act=nextfight"'
			value='<? echo $lang_comb["_conti"]; ?>'></td>
	</tr>
	<?php
		}
	}else{//player died
