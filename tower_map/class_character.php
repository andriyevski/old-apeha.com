...
class character
{
	...
	function attack_roll($comb_act,$test=false){
		...
		if ($comb_act == "magic_attack") {
			...
			if(DEBUG>=1) {$_SESSION['disp_msg'][] = "**DEBUG: fight reduction = $fight_reduction";}
			$char_attack = round(($this->dexterity
			+rand(
	...
	//      }
	//        $sql = "UPDATE players SET stamina = ".$this->stamina_points." WHERE id=$this->id";
	//        $res = mysql_query($sql);
	...
	function character($id){
		...
		$this->location_data= fetch_first("select * from tower_locations where id='$this->location'");
		...
		*/
	{
		...
		if($this->time_since_regen >= $data['healing_time'] && $data['healing_rate'] > 0)
		{
			...
			if ($this->hit_points < $this->max_hp){
				...
				$query = ("UPDATE players SET hit_points = '$char_regen_hp' WHERE id = '".$this->id."'");
				$req = @mysql_query($query);
				$this->hit_points=$char_regen_hp;
			}else{
				$query = ("UPDATE players SET  WHERE id = '".$this->id."'");
				$req = @mysql_query($query);
			}
		}
	}
		...
		*/
	{
		...
		if($this->stamina_time_since_regen >= $data['stamina_regen_time'] && $data['stamina_regen_rate'] > 0)
		{
			...
			if ($delta>0 && $this->stamina_points<$this->max_stamina)
			{
				...
				$query = ("UPDATE players SET stamina = '$char_regen', stamina_time = '".time()."'WHERE id = '".$this->id."'");
				$req = mysql_query($query);
				$this->stamina_points=$char_regen;
			}
			else
			{
				$query = ("UPDATE players SET stamina_time = '".time()."'WHERE id = '".$this->id."'");
				$req = mysql_query($query);
			}
		}
	}
	...
	function auto_reputation(){
		...
		if($this->rep_time_since_regen >= 86400) {
			...
			if ($this->rep_points < $this->max_rep){
				...
				$query = ("UPDATE players SET rep_points = '$rep_regen', rep_time = '".time()."' WHERE id = '".$this->id."'");
				$req = mysql_query($query);
				...
				$query = ("UPDATE players SET rep_points='$this->rep_points', rep_time = '".time()."' WHERE id = '".$this->id."'");
				$req = mysql_query($query);
				if (!$req) {showError(__FILE__,__LINE__,__FUNCTION__); exit;}
			}
		}
	...
	/**
	 * drinks an given Potion of the inventory
	 * @param $Inv_ID identifies the inventory-slot
	 */
	function drink_potion2($Inv_ID){
		$charID = $this->id;
		$result = mysql_query ("SELECT * FROM phaos_char_inventory WHERE ID='$Inv_ID'");
		if ($pot = mysql_fetch_array($result)) {
			...
			if ($row = mysql_fetch_array($result)) {
				...
				if ($effect == "heal" or $effect == "stamina") {
					...
					if(@$dbfield){
						...
						$sql="UPDATE players SET $dbfield = $current WHERE id = $charID";
	...
	function level_up($attribute){
		$query = ("UPDATE players SET stat_points = stat_points-1, $attribute = $attribute+5 WHERE id = '".$this->id."'");
		$req = mysql_query($query);
		if (!$req) {return(0);}
		else {return(1);}
	}
	...
	function kill_character(){
		//		$query = "DELETE FROM players WHERE user = '".$this->user."'";
	...
	function equipt($item_type,$item_id){
		$query = ("UPDATE players SET $item_type = '$item_id' WHERE id = '".$this->id."'");
		$req = mysql_query($query);
	...
	function unequipt($item_type){
		...
		$query = ("UPDATE players SET $item_type = '' WHERE id = '".$this->id."'");
		$req = mysql_query($query);
		if (!$req)  {showError(__FILE__,__LINE__,__FUNCTION__); return 0; exit;}
		return 1;
	}
	...
	function checkequipped($item_type,$item_id){
		...
		$res=mysql_query("SELECT * FROM phaos_char_inventory WHERE user = '".$this->user."' AND item_id = '$item_id' AND type = '$item_type'");
	...
	function reduce_stamina($ammount){
		...
		$query = ("UPDATE players SET stamina = '".$this->stamina_points."'
							   WHERE id = '".$this->id."'");
		$req = mysql_query($query);
	}
	...
	function pay($amount){
		...
		if($amount <= $this->gold) {
			...
			$req = mysql_query("UPDATE players SET gold = ".$this->gold." WHERE id = ".$this->id);
			if (!$req) {showError(__FILE__,__LINE__,__FUNCTION__); exit;}
			return 1;
		}	else {
			return 0;
	...
	function invent_count(){
		$result 	 = mysql_query("SELECT * FROM phaos_char_inventory WHERE user = '".$this->user."'");
	...
	function pickup_item($item){
		$pickedup= 0;
		if($item['type']=="gold") {
			$req = mysql_query("update players set gold=gold+$item[number] where id='$this->id'");
			if (!$req) {showError(__FILE__,__LINE__,__FUNCTION__); exit;}
			$pickedup+= 1;
		}else{
	...
	function skillup($skillname){
		...
		if ($total<(10+$this->level+$wisdomoffset)){
			...
			if ($rnd<$chance){
				$query="update players set $skillname=$skillname+1 where id='".$this->id."';";
	...
	function place($locationid=-1) {
        ...
        (  $idkey location,image_path,user,name,hit_points,stamina,stamina_time
        )
        VALUES
        (
        $idvalue '$this->location','$this->image','$this->user','$this->name','$this->hit_points',$this->stamina_points,$this->stamina_time
        )";
        ...
        if( $auto_increment_id ){
        	$query= "select id from players where location='$this->location' order by id desc LIMIT 1";
        	$req = mysql_query($query);
        	if (!$req) {showError(__FILE__,__LINE__,__FUNCTION__); exit;}
	...
	//FIXME: there should be a separate field in the database for characterrole, since right now it abuses the user
	...
	function sql_may_pass() {
		...
		$result = mysql_query ("SELECT id, buildings, pass FROM tower_locations WHERE id=$this->location AND $condition_pass");
	...
	function relocate($direction){
		...
		$query= "SELECT `$npc_dir` FROM tower_locations WHERE id=".$this->location;
		...
		if ( (@list($newloc)=mysql_fetch_array($result)) && $newloc ) {
			...
			$result = mysql_query ("SELECT id, buildings, pass FROM tower_locations WHERE id=$newloc AND $condition_pass");
			if ($row = mysql_fetch_array($result)) {
				/*OLD v0.89
...
class np_character_from_blueprint extends character {
	function np_character_from_blueprint($blueprint,$level=1,$user='phaos_npc'){
...
function npcgen() {
	...
	if($blueprint = mysql_fetch_array($res)) {
		...
		while($tries-->0){
			...
			if( !@$res && rand(0,99)<4 ) {
				$location= 'Rune Gate%';
				$sql = "SELECT id FROM tower_locations WHERE (name LIKE 'Rune Gate%' OR name LIKE 'Dungeon') AND $condition_passable ORDER BY RAND() LIMIT 1";
				//defined('DEBUG') and DEBUG and $GLOBALS['debugmsgs'][]= __FUNCTION__.": sql: $sql";
				$res = mysql_query ($sql) or die(mysql_error());
			}
			if( !@$res) {
				$location= 'Wilderness';
				$sql = "SELECT id FROM tower_locations WHERE (name LIKE 'Wilderness' OR name LIKE 'Woodlands' OR name LIKE 'Rune Gate%' OR name LIKE 'Dungeon') AND $condition_passable ORDER BY RAND() LIMIT 1";
				//defined('DEBUG') and DEBUG and $GLOBALS['debugmsgs'][]= __FUNCTION__.": sql: $sql";
				$res = mysql_query ($sql) or die(mysql_error());
			}
			...
			$res = mysql_query ("SELECT count(*) FROM players WHERE location='$locationid' AND user='phaos_npc'") or die(mysql_error());
			...
			if($count>$level+1) {
				...
				//trying to fix
				$res = mysql_query ("SELECT id FROM players WHERE location='$locationid' AND user='phaos_npc'") or die(mysql_error());
...
function getclan_sig($plname){
	...
	if ($clan_sig !== "no" or $clan_sig !== ""){
		?>
<img
	src="images/guild_sign/<? echo $clan_sig; ?>"
	alt="<? echo $clan_name; ?>">
		<?
	}
