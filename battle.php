...
// заплатка на окончание боя
if(empty($stat['last_battle']))$stat['last_battle']=$stat['battle'];
if($stat['battle']==0)$battle_id=$stat['last_battle'];
else $battle_id=$stat['battle']; 
...
if(!empty($ttarget)){
	...
	$targ=mysql_fetch_array(mysql_query("select hp,damaged_h,damaged_t,damaged_l,damaged_r,damaged_le from participants where time=".$battle_id.' AND id='.$targ['id']));
	include ('inc/battle/right.php');
	$self=mysql_fetch_array(mysql_query("select hp,damaged_h,damaged_t,damaged_l,damaged_r,damaged_le from participants where time=".$battle_id.' AND id='.$stat['id']));
...
//echo "doLoad('battle.php?ttarget=".$stat['user']."&tab=s&".rand()."');";
	...
	// переменные необходимые для подсчета ходов
	$offer=mysql_fetch_array(mysql_query("SELECT `timeout`, `type`, `blood`, `kulak`, `stavka`, `zone_width`, `zone_height` FROM offers WHERE time='".$battle_id."' LIMIT 1"));
	$max1=mysql_fetch_array(mysql_query("SELECT time,id FROM battles WHERE offer='".$battle_id."' AND type <> '0' ORDER BY `time` DESC LIMIT 1"));
	// ttarget
	$op_partic_count = @mysql_num_rows(mysql_query("select players.id from players,participants where players.battle='".$battle_id."' AND players.rank <> '60' AND players.hp_now > '0' AND participants.hp > '0' and players.id=participants.id AND participants.time='".$battle_id."' AND participants.frozen='0' and players.hp_now>0"));// считаем кол-во человек находящихся в бою
	$battles_sql = mysql_query("select id from battles where offer='".$battle_id."' AND type = '0' ORDER BY id DESC LIMIT 0,$op_partic_count");
	$op_bat_count = @mysql_num_rows($battles_sql); // считаем кол-во записей на ход

	include ('inc/battle/m_counting.php'); // файл с алгоритмом подсчета хода
	//
	echo 'timeout1='.(($offer['timeout']-(time()-$max1['time']))).';';
	
	if($op_partic_count==0){
	//mysql_query("update players set battle='0', last_battle='".$stat['battle']."' where where user='".$stat['user']."'");
		echo 'endbattle('.$battle_id.');';
    ...
    and players.hp_now > '0' 
        and participants.time='".$battle_id."' and participants.hp > '0'
        ");
	while($player=@mysql_fetch_array($players)){
		$player1=mysql_fetch_array(mysql_query("SELECT `type`,`x`,`y` FROM battles WHERE attacker='".$player['user']."' AND offer='".$battle_id."' ORDER by `time` DESC LIMIT 1"));
	...
	$all_obst = '';
	$obstacles = mysql_query('SELECT `c_x`,`c_y`,`img` FROM `obstacles` WHERE `offer_id`=\''.$battle_id.'\'');
