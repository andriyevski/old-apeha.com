...
if (!empty($sale) && is_numeric($sale)) {
	...
	$wmot = mysql_query("SELECT * FROM `items` WHERE `name` = '".$is_ex_inf[1]." AND art=1'");
	...
	if (!empty($is_ex_inf['0'])) {
		if (!$wmot) {
			if ($is_ex['tip'] != 13) {
				...
				mysql_query("UPDATE players SET valute=valute+".$price." WHERE id=".$stat['id']."");

				$stat['valute']+=$price;

				$msg="Вы удачно продали предмет <u>".$is_ex_inf['1']."</u> за <u>".$price."</u> сп.";

				$is_ex_inf['0'] = "";
			}
			else $msg="Предмет <u>".$is_ex_inf['1']."</u> не подледжит продаже!";
		}
		else $msg="Тип предмета не позволяет его продать!";
	}
