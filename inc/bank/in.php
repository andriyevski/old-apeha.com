...
if (is_numeric($tmp) && $tmp>0) {
	...
	if ((!empty($in)) && (!empty($ob['id']))) {mysql_query("UPDATE objects set bank=1 where id=$ob[id]");
	echo"<script>Header('Location: bank.php?set=edit&$now')</script>";
	echo"<br><center><font color=red><b>Вы успешно положили предмет в ячейку</b></font></center>"; }
}
?>
