...
document.all(\"battle_side\").innerHTML = '<center><br><table width=250 cellspacing=1 cellpadding=3 border=1 bgcolor=e2e0e0 bordercolor=CCCCCC><tr><td align=center style=\'BORDER-RIGHT: 0 px solid\' width=100%><b>Выбор группы</b></td><td width=20 align=center style=\'BORDER-LEFT: 0 px solid\'><span style=\'CURSOR: Hand\' onclick=\'document.all(\"battle_side\").style.display = \"none\";\'><b>X</b></span></td></tr><tr><td colspan=2><table width=100% cellspacing=0 cellpadding=0 border=0><tr><td width=49% align=center><input class=standbut value=\'Команда #1\' type=button onclick=\'window.location=\"battle.php?page=take_it&battle_type=2&offer='+battle_id+'&battle_side=0\"\'></td><td width=2% align=center><a class=ch>|</a></td><td width=49% align=center><input class=standbut value=\'Команда #2\' type=button onclick=\'window.location=\"battle.php?page=take_it&battle_type=2&offer='+battle_id+'&battle_side=1\"\'></td></tr></table></td></tr></table></center>';
	...
	if ($type==2) {
      ...
      "select players.user,players.id,players.level,players.rank,players.tribe,participants.side from participants, players
		...
		if (!$user_offer[time] && $user_offer[time]!=$offer[time]) {
			echo "<td align=center rowspan=3><img src='i/join.gif' onclick='if (confirm(\"Вы действительно хотите принять эту заявку?\")) battle_side(\"$offer[time]\");' style='CURSOR: Hand' alt='Принять вызов'></td>"; } else echo"<td rowspan=3>&nbsp;</td>";
...
<script language=JavaScript>show_inf('$participant[user]','$participant[id]','$participant[level]','$participant[rank]','$participant[tribe]');</script>";
      ...
      order by id");
				...
				} else echo"<center><i>Группа пока не набрана</i></center>";



				echo "</td></tr><tr>";

				echo"<td align=center>Бойцов: <b>".$offer['size_left']."</b> <a class=ch>|</a> Уровень: [ <b>".$offer['level_l_min']."-".$offer['level_l_max']."</b> ]</td>";

				echo"<td align=center>Бойцов: <b>".$offer['size_right']."</b> <a class=ch>|</a> Уровень: [ <b>".$offer['level_r_min']."-".$offer['level_r_max']."</b> ]</td>";
...
<script>ShowTime('battle_start_$offer[time]',",($offer[start]-$now)+rand(-2,10),",1);</script>
</b></td>

<td valign=center><small>&nbsp;]</small>";

				// if (!empty($offer['comment'])) echo"<br></font> <small>[ <i><b>$offer[comment]</b></i> ]</small>";
