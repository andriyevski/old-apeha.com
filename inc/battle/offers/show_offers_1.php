...
echo"<div id=battle_forms>132</div>";
...
<script language=JavaScript>show_inf('$participant[user]','$participant[id]','$participant[level]','$participant[rank]','$participant[tribe]');</script>";
      ...
      "select participants.*, players.user,players.level,players.rank,players.tribe from participants, players
         ...
         order by id");
			...
			if (!empty($offer['comment'])) echo" <font color=blue> <small>[ <i><b>$offer[comment]</b></i> ]</font></small>";
			if (!empty($offer['blood'])) echo" <img title=\"Кровавый бой\" src='i/ckelet.gif'>";
			if (!empty($offer['kulak'])) echo" <img title=\"Кулачный бой\" src='i/kulak.gif'>";
