<?
if ($access[$p] == 1) {
	$text = "<script language=JavaScript>show_inf(\"".$stat['user']."\",\"".$stat['id']."\",\"".$stat['level']."\",\"".$stat['rank']."\",\"".$stat['tribe']."\");</script> применил прием <b>\"Сильный Удар\"</b><HR COLOR=e2e0e0>";
	$max=mysql_fetch_array(mysql_query("select max(id) as id from battles where offer='".$stat['battle']."'"));
	if (!$max) $new_id=1;
	else $new_id=$max['id']+1;
	if ($stat[level] == 0) {
		$add_dmg_by_lvl = (int)($add_dmg_lvl3/2);
	}
	else {
		$add_dmg_by_lvl = $stat[level]*$add_dmg_lvl3;
	}
