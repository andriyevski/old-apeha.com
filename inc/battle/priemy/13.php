<?
if ($access[$p] == 1) {
	$text = "<script language=JavaScript>show_inf(\"".$stat['user']."\",\"".$stat['id']."\",\"".$stat['level']."\",\"".$stat['rank']."\",\"".$stat['tribe']."\");</script> применил прием <b>\"Воля к победе \"</b><HR COLOR=e2e0e0>";
	...
	if ($stat[hp_now] <= ((int)($stat[hp_max]/3))) {
		$add_hp = $stat[hp_now] + $stat[level]*5;
		$stat[hp_now]+=$stat[level]*5;
