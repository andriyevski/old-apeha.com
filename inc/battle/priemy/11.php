...
if ($access[$p] == 1) {
	...
	if ($stat[level] == 0) {
		$add_dmg_by_lvl = (int)($add_dmg_lvl2/2);
	}
	else {
		$add_dmg_by_lvl = $stat[level]*$add_dmg_lvl2;
	}
	...
	if (mysql_num_rows($inf_sel) == 1) {
		...
		(1+$stat[strength]/1.5+$stat[max])*(1+($stat['ork']/100)))/3);
		$add_hp = $info[hp_now] - $add_dmg_by_lvl;
		$text = "<script language=JavaScript>show_inf(\"".$stat['user']."\",\"".$stat['id']."\",\"".$stat['level']."\",\"".$stat['rank']."\",\"".$stat['tribe']."\");</script> применил прием <b>\"Прорыв\"</b> и тем самым нанес <b>$add_dmg_by_lvl</b> урона пермонажу <b>$enemy</b><HR COLOR=e2e0e0>";
