...
elseif ($chl['t_level'] > 1) $nms="Таким свитком данную травму не вылечить!";
elseif ($stat['energy_now'] < 10) $nms="У вас не хватает маны!";
elseif ($chl['room'] != $stat['room']) $nms="Для исцеления персонажа от травм Вам необходимо находится в одной комнате!";
	...
	if ($chl['user'] == $stat['user']) {
		$MesgForAdd = "Вы успешно исцелили себя от лёгких травм!";
		$stat['travma'] = NULL;
	}
	else $MesgForAdd = "Вы исцелены от лёгких травм лекарем <b><u>".$stat['user']."</u></b>";

	require_once("function/chat_insert.php");
	insert_msg("$MesgForAdd","","","1",$chl[user],"","0");

	$nms="Всё прошло удачно...<br>Персонаж <u>".$chl['user']."</u> успешно исцелён от лёгких травм!";
	$alldone=1;
}
