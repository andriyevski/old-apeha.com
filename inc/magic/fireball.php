...
if ($iteminfo['name'] == "fireball30") {$for_add1 = 30+intval(30*$addd); $energy = 10;}
elseif ($iteminfo['name'] == "fireball40") {$for_add1 = 40+intval(40*$addd); $energy = 15;}
elseif ($iteminfo['name'] == "fireball50") {$for_add1 = 50+intval(50*$addd); $energy = 20;}
elseif ($iteminfo['name'] == "fireball65") {$for_add1 = 65+intval(65*$addd); $energy = 25;}
...
elseif ($stat['ustal_now'] <= $energy) $nms="У вас не хватает маны!";
	...
	if ($chl['hp_now'] - $for_add <= 0){
		...
		$hpplus=$chl['hp_now'];}
			...
			if ($stat[user]!="$chl[user]") $MesgForAdd = "Маг <b><u>$stat[user]</u></b> выпустил в вас огненный шар... Огонь поглотил ваши  жизни <b><u>-$hpplus НР<u></b>";
			else $MesgForAdd = "Наверное вы  недостаточно  обучены магии... Вы подожгли сами себя на ... : <b><u>-$hpplus НР</u></b>";

			include("inc/magic/drop.php");

			require_once("inc/chat/functions/functions.php");
			...
			mysql_query("update players set ustal_now=ustal_now-$energy where id='".$stat['id']."'");

			$nms="Свиток магии огня прочитан ...<br>Персонажа <u>".$chl['user']."</u> ударило огненным шаром и он обгорел на <u>-$hpplus HP</u>";
