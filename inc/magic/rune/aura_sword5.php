...
if ($stat['aura_t'] > $now) $nms="¬ы можете находитс€ под действием только одной ауры!";
	...
	mysql_query("UPDATE `person` SET `aura`='10', `aura_t`='$aura_time' WHERE `user`='".$stat['user']."'");

	require_once("function/chat_insert.php");
	insert_msg("Ќа вас была наложена аура зачарованный меч 5 уровн€!","","","1",$stat['user'],"",$stat['room']);
