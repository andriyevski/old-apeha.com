		...
		if (mysql_num_rows($buyitem_res)) {
			...
			if ($buyitem['price']<=$stat['credits']) {
				...
				if ($shop_sost['kol']>0) {
					...
					$result = mysql_query("UPDATE `shop`,`players`,`akcii` SET shop.kol=shop.kol-1, players.credits = players.credits - ".$buyitem['price'].", akcii.all_money = akcii.all_money + ".$buyitem['price'].", akcii.money = akcii.money + ".$buyitem['price']." WHERE akcii.id = '2' and shop.name = '".addslashes($_GET['buy'])."' && players.user = '".$stat['user']."' AND players.credits>=".$buyitem['price']."");
