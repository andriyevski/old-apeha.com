		...
		if (mysql_num_rows($buyitem_res)) {
			...
			if ($buyitem['price']<=$stat['valute']) {
				...
				if ($shop_sost['kol']>0) {
					...
					$result = mysql_query("UPDATE `shop`,`players` SET shop.kol=shop.kol-1, players.valute = players.valute - ".$buyitem['price']." WHERE shop.name = '".addslashes($_GET['buy'])."' && players.user = '".$stat['user']."' AND players.valute>=".$buyitem['price']."");
					...
					if ($result) {
						...
						if ($result2) {
							...
							$msg="Вы купили предмет <u>".$buyitem['title']."</u> за <u>".$buyitem['price']."</u> сп.";
						}
					}
				}
			}
