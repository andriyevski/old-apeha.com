								...
								if ( $law == 1 ) {
									if ( $_POST['right_code'] == $_POST['number'] ) {
										if ( $reg_ip == 0 ) {
											...
											$norm="Вы удачно зарегистрировались под логином: ".addslashes($login)."<br>Ваш пароль от перонажа: $psw <br> Можете закрыть это окошко, и открыть предидущее... http://mil.yard.ru/  и выполнять следующее действие по инструкции";

										}

										else $msg="Ошибка! Регистрация доступна 1 раз в день!"; }
										...
										else $msg="Ошибка! Пароли слишком мал!"; }
										else $msg="Ошибка! Пароли не совпадают!"; }
										else $msg="Ошибка! Логин содержит недопустимые символы!"; }
										else $msg="Ошибка! Логин менее 3х символов или более 10 символов!"; }
