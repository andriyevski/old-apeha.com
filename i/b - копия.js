var by,bx,bcid,tcid,nx,ny,sx,sy,tname,last_id,wait,mteam,mname,zx,zy,tx,ty,bl=[],redirect;
by,bx=-1,-1;
...
if(parseInt(dist(sx,sy,tx,ty))>1||document.getElementById('buttons').style.visibility=='hidden') document.getElementById('striker').style.visibility='hidden'; else document.getElementById('striker').style.visibility='visible';
doLoad('battle.php?ttarget='+tname+'&'+Math.random()*1000000000000);
}
...
function strikec(){
document.getElementById('gamep').innerHTML='<form method=POST action=\'battle.php?page=battle\'><input type=hidden name=opponent value=\''+tname+'\'><table width=178 height=111 bgcolor=#e5e2d4><tr><td background="img/doll.gif" valign=top width=79><div style="position:relative;left:1px;"><input type=checkbox name=rkick value=1 style="position:absolute; top:-1px; left:20px;" onclick=\'epic(this);\' id=\'bc\'><input type=checkbox name=lkick value=1 style="position:absolute; top:-1px; left:32px;" onclick=\'epic(this);\' id=\'bc\'><input type=checkbox name=rkick value=2 style="position:absolute; top:22px; left:20px;" onclick=\'epic(this);\' id=\'bc\'><input type=checkbox name=lkick value=2 style="position:absolute; top:22px; left:32px;" onclick=\'epic(this);\' id=\'bc\'><input type=checkbox name=rkick value=3 style="position:absolute; top:42px; left:-5px;" onclick=\'epic(this);\' id=\'bc\'><input type=checkbox name=lkick value=3 style="position:absolute; top:42px; left:7px;" onclick=\'epic(this);\' id=\'bc\'><input type=checkbox name=rkick value=4 style="position:absolute; top:42px; left:46px;" onclick=\'epic(this);\' id=\'bc\'><input type=checkbox name=lkick value=4 style="position:absolute; top:42px; left:58px;" onclick=\'epic(this);\' id=\'bc\'><input type=checkbox name=rkick value=5 style="position:absolute; top:72px; left:20px;" onclick=\'epic(this);\' id=\'bc\'><input type=checkbox name=lkick value=5 style="position:absolute; top:72px; left:32px;" onclick=\'epic(this);\' id=\'bc\'></div></td><td width=20></td><td background="img/doll.gif" valign=top width=79><div style="position:relative;left:1px;"><input type=checkbox name=rblock[] value=1 style="position:absolute; top:-1px; left:20px;" onclick=\'epic(this);\' id=\'bc\'><input type=checkbox name=lblock[] value=1 style="position:absolute; top:-1px; left:32px;" onclick=\'epic(this);\' id=\'bc\'><input type=checkbox name=rblock[] value=2 style="position:absolute; top:22px; left:20px;" onclick=\'epic(this);\' id=\'bc\'><input type=checkbox name=lblock[] value=2 style="position:absolute; top:22px; left:32px;" onclick=\'epic(this);\' id=\'bc\'><input type=checkbox name=rblock[] value=3 style="position:absolute; top:42px; left:-5px;" onclick=\'epic(this);\' id=\'bc\'><input type=checkbox name=lblock[] value=3 style="position:absolute; top:42px; left:7px;" onclick=\'epic(this);\' id=\'bc\'><input type=checkbox name=rblock[] value=4 style="position:absolute; top:42px; left:46px;" onclick=\'epic(this);\' id=\'bc\'><input type=checkbox name=lblock[] value=4 style="position:absolute; top:42px; left:58px;" onclick=\'epic(this);\' id=\'bc\'><input type=checkbox name=rblock[] value=5 style="position:absolute; top:72px; left:20px;" onclick=\'epic(this);\' id=\'bc\'><input type=checkbox name=lblock[] value=5 style="position:absolute; top:72px; left:32px;" onclick=\'epic(this);\' id=\'bc\'></div></td></tr></table></center><input type=\'button\' value=\'Вернутся\' onclick=\'forceupd()\' class=standbut><input type=submit value=\'Удар\' class=standbut name=fight onClick="fight.disabled = true; document.forms[0].submit()"></form></center>';
...
document.getElementById('map').innerHTML+='<img src=\"img/obst/1.gif\" style=\"border=0;position:absolute;left:'+(x*34+17*(y%2!=0))+'px;top:'+(y*30)+'px;\">';
}

function moveto(){
  if((Math.abs(nx-sx)+Math.abs(ny-sy)<2)||(sy%2==0 && nx==sx-1 && Math.abs(sy-ny)==1)||(sy%2!=0 && nx==sx+1 && Math.abs(sy-ny)==1)){
...
function view_item_b (name,slot,w,h,hint,br,title,id) {
// alert(slot);
if(slot=='w1' || slot=='w3' || slot=='w4' || slot=='w13') elid='twleft'; else{
if(slot!='w18' && slot!='w17') elid='twright'; else 
if(slot=='w17') elid='lpacket'; else elid='rpacket';
}
...
function epic(num){
   ...
   bl['l']=0;
   bl['r']=0;
   for(i=0;i<r.length;i++)if(r[i].name=='lblock[]' && r[i].checked) bl['l']++; else if(r[i].name=='rblock[]' && r[i].checked) bl['r']++;
...
function sdll(targdoll,head,torso,right,left,legs){
