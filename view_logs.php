...
if ($log!="") {
	...
	$last_turns=mysql_query("select id, time, attacker, defender, comment1,comment2, type from battles where offer='".addslashes($log)."' AND (comment1 IS NOT NULL OR comment2 IS NOT NULL)  ORDER BY id DESC, time ASC");
	...
	echo '<center><input type=button value=\'Обновить\' onclick=\'disabled = true; window.location.href="view_logs.php?log='.$log.'"\' class=input></center>
	<table width=700 align=center border=0 cellspasing=5 cellpadding=5 >';
	if ($last_turns)
